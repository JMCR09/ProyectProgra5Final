...
namespace Incidentes.Model
{
    public class Datos
    {
        ...
        public Datos()
        {
            strCadenaConexion = @"Data Source= ProyecProgra5; Initial Catalog = Incidentes; Integrated Security=SSPI";
        }

       public string Conexion()
        {
            string con = @"Data Source= ProyecProgra5; Initial Catalog = Incidentes; Integrated Security=SSPI";
        ...
        {
            //Para hacer los INSERT - DELETE - UPDATE
             //dtDatos = new DataSet();
        ...
        #region
        public bool RetornaValor(SqlParameter[] parParametros, string parSPName)
        {
            ...
            try
            {
                ...
                cmdComando.Parameters.AddRange(parParametros);
                //Ejecutamos el TSQL(Transaction SQL) en el servidor.
                object resultadoObject = cmdComando.ExecuteScalar();
                if (resultadoObject != null && resultadoObject != DBNull.Value)
                {
                    resultado = Convert.ToBoolean(resultadoObject);
                }
            }
            catch (Exception ex)
            {
